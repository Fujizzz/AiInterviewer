...
统一入口为 `agents/question/quality.py`，接在 ReAct final 的结构校验之后。出题前依据当前线程的 `answer_scope` 和缺失信息提示模型选择一个具体的信息点；出题后调用一次结构化语义审查，检查以下四类问题：

| 问题 | 处理 |
| --- | --- |
| 换词重问 | 比较当前项目已问问题的实际信息要求；允许把宽泛问题收窄为个人负责的一个部分 |
| 一题多问 | 检查独立回答要求，不以问号数量代替语义检查；模糊回答后应缩小问题范围 |
| 内部规则泄露 | 显式题数规则由程序拦截，其他表达由模型审查；技术资源预算可以正常讨论 |
| 虚构经历前提 | 依据所选项目的简历声明和当前线程回答；开放询问未知细节、明确的假设题不算虚构经历 |
| 话题不一致 | 同时检查问题正文和信息目标；同项目存在 Transformer 经历，不代表选择 Ray 话题时可以继续问 Transformer |

审查模型不读取评分或预算元数据，只得到项目依据、同项目已问问题及当前线程问答。切换话题时不附带旧线程答案；旧问题仅用于去重，不能作为事实依据。历史缺失或截断时不能凭空补全事实。

审查只返回问题代码和简短修复建议。未通过时把草稿及建议交回原 Question Agent，修复后的问题再次检查。`retries.llm_generation_retries` 默认 3：首次生成之外，结构错误、质量不通过和审查接口异常共用最多三次重试额度，每次出题尝试最多四次质量审查，不增加追问计数，也不强制工具调用。重试额度耗尽仍未通过或检查仍不可用时，使用现有项目上下文模板兜底；超时仍提前兜底，取消仍直接停止。日志明确区分兜底与通过审查的问题。

审查接口异常时重审同一草稿，不重新出题；只有有效的不通过结论才交给出题模型改写。两者共用三次重试上限。审查建议超过长度时保留问题代码并裁剪说明，决策摘要同样归一化；问题代码、ID 和动作等控制字段仍严格校验。

通常每题增加一次模型调用。`question_agent.quality_timeout_seconds` 默认 20 秒，与生成、工具及修复共同受整轮 90 秒上限约束。质量日志仍写入同一个面试 Markdown 文件，拒绝时额外保留草稿和简短修复建议，不展示在面试终端，不记录内部思维链。

兜底不再从诊断字符串匹配 architecture/component 等关键词，而是在当前项目与话题中使用责任范围、一个具体任务或一个实施步骤作为恢复提问，并把实际目标写回问题元数据。它仍是模板降级，不等同于通过语义审查。

语义审查存在误判、漏判风险；禁用 ReAct 的旧生成路径与离线模板兜底仅做结构检查。离线测试中的审查结论是预设的，用来验证控制流程。真实模型样例验证可运行：

```powershell
.\.venv\Scripts\python.exe -m docs.examples.verify_question_quality
```

该命令读取现有 `.env` 配置并产生 API 调用，使用预设项目和回答检查四类错误、有效收窄、开放提问和假设题，再注入一次重复草稿验证真实修复。结果保存为 `output/提问质量真实模型验证.md`；它不代替完整面试或评分验证。

完整真实模型回放：`.\.venv\Scripts\python.exe -m docs.examples.replay_interview .\Wang_Shunyao_CV.pdf`。使用历史日志中的八条回答作为输入；简历解析、出题、审查、回答评价和报告均调用真实模型。每轮保存一个标注为测试回放的面试 Markdown。预设回答仅用来复现失败条件，不能当成新的候选人评价。
