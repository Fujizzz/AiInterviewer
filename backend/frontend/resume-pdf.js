/**
 * @module resume-pdf
 * 职责：上传 PDF、展示真实进度、最终文本与必要疑点，人工采用后失效既有简历预解析。
 * 实现：fetch 读取 NDJSON，取消使用 AbortController；与面试状态通过 DOM 禁用属性联动。
 * 关联：agent.html、/api/resume/parse/；不将 PDF 放进 WebSocket，不持久化文件或文本。
 * 目录：
 * - pdfEl：查找模板元素。
 ...
 * - pdfRun：上传文件并按行消费 UTF-8 流，拒绝缺失终态。
...
function pdfReset() {
  ...
  pdfEl("pdf-status").textContent = "文件已选择，点击后将依次执行传统提取和视觉校对。";
...
function pdfCancel() {
  if (pdfController) {
    ...
    pdfController = null;
    pdfReviewed = false;
    pdfEl("pdf-status").textContent = "已取消，尚未完成视觉校对。";
  }
...
function pdfEvent(data) {
  ...
  if (data.type === "progress") pdfEl("pdf-status").textContent = data.detail;
  else if (data.type === "result") {
    pdfEl("pdf-preview").value = data.text;
    pdfEl("pdf-panel").hidden = false;
    const notes = [];
    for (const page of data.pages) {
      if (page.uncertainties.length) notes.push(`第 ${page.number} 页待核对：${page.uncertainties.join("；")}`);
    }
    pdfEl("pdf-notes").textContent = notes.join("\n");
    pdfEl("pdf-status").textContent = "校对完成，可直接使用下方结果。";
    return true;
  }
  return false;
}

/** 无外部参数；单次上传依次提取和校对，完整成功后允许采用，失败不重发。 */
async function pdfRun() {
  if (pdfController || pdfEl("resume").disabled) return;
  const file = pdfEl("pdf-file").files[0];
  if (!file) return;
  if (!file.size || file.size > 10 * 1024 * 1024) {
    pdfEl("pdf-status").textContent = "PDF 必须非空且不超过 10 MiB。";
    return;
  }
  pdfReset();
  ...
  pdfEl("pdf-status").textContent = "正在上传 PDF…";
  pdfControls();
  ...
  try {
    ...
    while (true) {
      ...
      while ((newline = buffer.indexOf("\n")) >= 0) {
        ...
        if (line.trim()) {
          if (completed) throw new Error("完成事件后收到额外数据。");
          completed = pdfEvent(JSON.parse(line));
        }
      }
      if (done) break;
    }
    if (!completed || buffer.trim()) throw new Error("解析连接提前结束，结果未完成。");
    pdfReviewed = true;
  } catch (error) {
    if (pdfController === controller) pdfEl("pdf-status").textContent = `解析失败：${error.message}（未自动采用文本）`;
  } finally {
    if (reader) {
...
function pdfUse() {
  ...
  pdfEl("pdf-status").textContent = "已填入简历文本，可继续编辑、提前解析或开始面试。";
  pdfEl("resume").focus();
}
...
function pdfClear() {
  ...
  pdfEl("pdf-status").textContent = "选择 PDF 后解析并校对；也可以直接粘贴文本。";
  pdfControls();
}
